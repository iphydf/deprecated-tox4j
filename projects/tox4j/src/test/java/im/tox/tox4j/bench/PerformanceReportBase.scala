...
object PerformanceReportBase {
   ...
   */
  def friendAddresses(sz: Int): Seq[FriendAddress] = {
   ...
   */
  def friendKeys(sz: Int): Seq[PublicKey] = {
    for (_ <- 0 until sz) yield {
      ...
      key(key.length - 1) = 0
      PublicKey.unsafeFromByteArray(key)
    }
  }
  ...
  def makeToxWithFriends(friendCount: Int): ToxCore[Unit] = {
    ...
    tox.setName(Nickname.unsafeFromByteArray(Array.ofDim(ToxCoreConstants.MaxNameLength)))
   ...
   * @return A pair containing the passed Tox instance and a random slice of the friend list.
   */
  def toxAndFriendKeys(limit: Int)(tox: ToxCore[Unit]): (Seq[PublicKey], ToxCore[Unit]) = {
  ...
  val names = nameLengths.map(Array.ofDim[Byte]).map(Nickname.unsafeFromByteArray)
