 ...
 *
 * This interface is designed to be thread-safe. However, once [[ToxCoreT.close]] has been called, all subsequent calls
 * will result in [[im.tox.tox4j.exceptions.ToxKilledException]] being thrown. When one thread invokes
 * [[ToxCoreT.close]], all other threads with pending calls will throw. The exception is unchecked, as it should not occur
 * in a normal execution flow. To prevent it from occurring in a multi-threaded environment, all additional threads
 * should be stopped or stop using the instance before one thread invokes [[ToxCoreT.close]] on it, or appropriate
 * exception handlers should be installed in all threads.
 *
 * The trait is parametric in a result wrapper type. See [[IdentityResult]] and [[wrapper.DisjunctionResult]]
 * for examples.
 *
 * @tparam ToxCoreState An arbitrary state type that is passed to and returned by [[ToxCoreT.iterate]] and is
 *                      passed through all callbacks.
 * @tparam Result The return type wrapper for all methods that can throw exceptions.
 * @tparam Self The derived type itself, used for [[ToxCoreT.load]], which returns a new instance of the same type as
 *              the instance it was called on.
 */
trait ToxCoreT[ToxCoreState, Result[E <: Enum[E], _], Self <: ToxCoreT[ToxCoreState, Result, Self]] extends Closeable {
  ...
  /**
   * Create a new [[Self]] instance with different options. The implementation may choose to create an object of
   * its own class or a derived class. If the implementation was compatible with another subsystem implementation (e.g.
   ...
   *
   * @return a new [[ToxCoreT]] instance.
   */
  @NotNull
  @throws[ToxNewException]
  def load(@NotNull options: ToxOptions): Result[ToxNewException.Code, Self]
  ...
  ): Result[ToxBootstrapException.Code, Unit]
  ...
  def getUdpPort: Result[ToxGetPortException.Code, Port]
   ...
   */
  @throws[ToxGetPortException]
  def getTcpPort: Result[ToxGetPortException.Code, Port]
  ...
  def setName(@NotNull name: ToxNickname): Result[ToxSetInfoException.Code, Unit]
   ...
   */
  @throws[ToxSetInfoException]
  def setStatusMessage(@NotNull message: ToxStatusMessage): Result[ToxSetInfoException.Code, Unit]
  ...
  def addFriend(
    @NotNull address: ToxFriendAddress,
    @NotNull message: ToxFriendRequestMessage
  ): Result[ToxFriendAddException.Code, Int]
  ...
  def addFriendNorequest(@NotNull publicKey: ToxPublicKey): Result[ToxFriendAddException.Code, Int]
  ...
  def deleteFriend(friendNumber: Int): Result[ToxFriendDeleteException.Code, Unit]
  ...
  def friendByPublicKey(@NotNull publicKey: ToxPublicKey): Result[ToxFriendByPublicKeyException.Code, Int]
  ...
  def getFriendPublicKey(friendNumber: Int): Result[ToxFriendGetPublicKeyException.Code, ToxPublicKey]
  ...
  def setTyping(
    friendNumber: Int,
    typing: Boolean
  ): Result[ToxSetTypingException.Code, Unit]
  ...
  def friendSendMessage(
    friendNumber: Int,
    @NotNull messageType: ToxMessageType,
    timeDelta: Int,
    @NotNull message: ToxFriendMessage
  ): Result[ToxFriendSendMessageException.Code, Int]
  ...
  ): Result[ToxFileControlException.Code, Unit]
  ...
  def fileSeek(
    ...
    fileNumber: Int,
    position: Long
  ): Result[ToxFileSeekException.Code, Unit]
  ...
  def getFileFileId(
    friendNumber: Int,
    fileNumber: Int
  ): Result[ToxFileGetException.Code, ToxFileId]
  ...
  def fileSend(
    ...
    kind: Int,
    fileSize: Long,
    @NotNull fileId: ToxFileId,
    @NotNull filename: ToxFilename
  ): Result[ToxFileSendException.Code, Int]
  ...
  ): Result[ToxFileSendChunkException.Code, Unit]
  ...
  def friendSendLossyPacket(
    friendNumber: Int,
    @NotNull data: ToxLossyPacket
  ): Result[ToxFriendCustomPacketException.Code, Unit]
